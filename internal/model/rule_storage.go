...
type RuleStorageCondition struct {
	...
	RuleBeginTime       global.Timestamp `gorm:"column:rule_begin_time;comment:规则开始时间" json:"rule_begin_time"`               // 规则开始时间
	RuleEndTime         global.Timestamp `gorm:"column:rule_end_time;comment:规则结束时间" json:"rule_end_time"`                   // 规则结束时间
}
...
type RuleStorage struct {
	...
	CreateTime global.Timestamp `gorm:"column:create_time;comment:创建时间" json:"create_time"` // 创建时间
	UpdateTime global.Timestamp `gorm:"column:update_time;comment:更新时间" json:"update_time"`
}
...
func (m *RuleStorage) FormatList(list []RuleStorage, c *gin.Context) {
	...
	userInfos := rpc.Auth.GetUserInfo(c.Request, userIds)
	for k := range list {
		list[k].UserName = userInfos[strconv.Itoa(int(list[k].UserID))]["name"]
	}
}

func (m *RuleStorage) queryList(param ExtRuleStorage) *gorm.DB {
	query := global.GormPg.Table("rule_storage ll").Where("ll.id>0")
	FilterWhere(query, "ll.id=?", param.ID)
	FilterWhere(query, "ll.specialty=?", param.Specialty)
	FilterWhere(query, "ll.net_manage=?", param.NetManage)
	FilterWhere(query, "ll.city=?", param.City)
	FilterWhere(query, "ll.area=?", param.Area)
	FilterWhere(query, "ll.alarm_level=?", param.AlarmLevel)
	FilterWhere(query, "ll.device_name=?", param.DeviceName)
	FilterWhere(query, "ll.device_type=?", param.DeviceType)
	FilterWhere(query, "ll.alarm_sub_type=?", param.AlarmSubType)
	FilterWhere(query, "ll.include_alarm_keyword=?", param.IncludeAlarmKeyword)
	FilterWhere(query, "ll.exclude_alarm_keyword=?", param.ExcludeAlarmKeyword)
	FilterWhere(query, "ll.way=?", param.Way)
	FilterWhere(query, "ll.order_delay=?", param.OrderDelay)
	FilterWhere(query, "ll.rule_begin_time=?", param.RuleBeginTime)
	FilterWhere(query, "ll.rule_end_time=?", param.RuleEndTime)
	FilterWhere(query, "ll.priority=?", param.Priority)
...
func (m *RuleStorage) ExcelExport(param ExtRuleStorage, c *gin.Context) (string, error) {
	// 1 名称和标题
	return ExcelExport("入库规则", []interface{}{"规则ID", "专业", "网管", "地市区域", "区县（子区域）", "告警级别", "设备名称", "设备类型", "告警子类型", "包含告警关键字", "不包含告警关键字", "规则开始时间", "规则结束时间", "处理方式", "派单延迟时间（分钟）", "优先级"},
		func(page int, pageSize int, isCount bool) excelReadTable {
			param.Page = page
			param.PageSize = pageSize
			result := m.List(param, isCount, c)
			return excelReadTable{result["count"].(int64), result["list"]}
		}, func(stream *excelize.StreamWriter, table excelReadTable, rowNext *int) {
			var row []interface{}
